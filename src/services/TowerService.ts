...
import type { OwnedItem, FusionTier } from '../types';
...
export class TowerService {
  ...
  /** Is the cell full? (max 2 towers, or 1 fused tower occupies the whole cell) */
  isCellFull(towers: BaseTower[], col: number, row: number): boolean {
    const here = this.towersAt(towers, col, row);
    if (here.some(t => t.fusionDef)) return true;  // fused tower blocks the cell
  ...
  /**
   * Check if two towers on a cell can fuse and at which tier.
   * Returns 'early' (both level 11+), 'late' (both level 21), or false.
   * Requires: neither tower evolved, primary not already fused, valid pair.
   */
  canFuse(towers: BaseTower[], col: number, row: number): FusionTier | false {
    ...
    // Mutual exclusivity: cannot fuse if either tower has evolved
    if (primary.evolutionDef || secondary.evolutionDef) return false;
    if (!fusionRegistry.has(primary.def.element, secondary.def.element)) return false;

    const cfg = GameConfig.get().tower;
    if (primary.upgradeCount >= cfg.lateFusionLevel && secondary.upgradeCount >= cfg.lateFusionLevel) {
      return 'late';
    }
    if (primary.upgradeCount >= cfg.earlyFusionLevel && secondary.upgradeCount >= cfg.earlyFusionLevel) {
      return 'early';
    }
    return false;
  }
  ...
  fuseTowers(ctx: IGameContext, col: number, row: number): boolean {
    const tier = this.canFuse(ctx.towers, col, row);
    ...
    const cfg = GameConfig.get().tower;
    const powerScalar = tier === 'late' ? cfg.lateFusionPowerScalar : cfg.earlyFusionPowerScalar;

    const ft = new FusionTower(primary.def, primary.gridX, primary.gridY, fusion, tier, powerScalar);
    ...
    const tierLabel = tier === 'late' ? '125%' : '75%';
    ctx.addFT(
      { x: ft.pixelX, y: ft.pixelY - 30 },
      `${fusion.icon} FUSÃO: ${fusion.name}! (${tierLabel})`, fusion.color,
    );
    ctx.addFT(
  ...
  moveTower(ctx: IGameContext, tower: BaseTower, col: number, row: number): boolean {
    ...
    const totalMoveCost = this.getMoveCost(cellTowers);
